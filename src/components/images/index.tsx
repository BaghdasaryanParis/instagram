...
export default function Images({ image }: Image) {
  ...
  const handleAddComment = () => {
    ...
    fetch(`http://localhost:5000/comments/${image.id}`, {
      method: "POST",
      headers: {
  ...
  console.log(image.comments[0]?.user.username);

  const handleLikedComment = async () => {
    const token = localStorage.getItem('token');
    try {
      const method = isLiked ? 'DELETE' : 'POST';
      const res = await fetch(`http://localhost:5000/post/${image.id}/like`, {
        method,
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      {image.imageUrl && (
        <div className="relative">
          ...
          <div className="flex space-x-2 mt-2">
            {isLiked ? <img onClick={handleLikedComment} className="w-8 h-8 cursor-pointer" src={heart.src} alt="love" /> :<img onClick={handleLikedComment} className="w-8 h-8 cursor-pointer" src={love.src} alt="heart" />}
            <span>{count}</span>
            <img
